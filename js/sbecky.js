...
// debug variables
_sbecky_debug_ajax = false
_sbecky_debug_register = true
_sbecky_debug_unregister = true
...
function _sbecky_on_change(variable){
    return function(value) {
        for (var dom of _sbecky_bindings[variable]) {
            // console.log(_sbecky_bindings[variable].length, _sbecky_bindings[variable]);
...
function _sbecky_register(variable, object) {
    ...
    if(_sbecky_debug_register) console.log("sbecky: register " + variable);
...
// unbind old dom elements by name
function _sbecky_unregister(name) {
    var els = sbecky_get(name);
    if(els && els.length > 0)
        _sbecky_unbind(els[0]);
}

// unbind elements contained in data
function _sbecky_unbind(data) {
    ...
    for(var item of all["[sb-button]"]) {
        if(_sbecky_debug_unregister) console.log("sbecky: unregister button", item.getAttribute("sb-button"));
        _sbecky_buttons[item.getAttribute("sb-button")] = undefined
    }

    for(var item of all["[sb-bind]"]) {
        if(_sbecky_debug_unregister) console.log("sbecky: unregister", item.getAttribute("sb-bind"));
        var index = _sbecky_bindings[item.getAttribute("sb-bind")].indexOf(item)
        if(index > -1) {
            _sbecky_bindings[item.getAttribute("sb-bind")].splice(index);
        };
        // check if sbecky[variable] can be deleted
        if (!_sbecky_bindings[item.getAttribute("sb-bind")]
                || _sbecky_bindings[item.getAttribute("sb-bind")].length <= 0) {
            sbecky[item.getAttribute("sb-bind")] = {}
        }
    }

    for(var item of all["[sb-form]"]) {
        if(_sbecky_debug_unregister) console.log("sbecky: unregister form", item.getAttribute("sb-form"));
        _sbecky_forms[item.getAttribute("sb-form")] = undefined
    }

    for(var item of all["[sb-ajax]"]) {
        if(_sbecky_debug_unregister) console.log("sbecky: unregister ajax", item.getAttribute("sb-ajax"));
...
function _sbecky_ajax_wrapper(it, params=undefined, success=undefined, error=undefined) {
    ...
    if(params["items"] == undefined/* || ids.length < parseInt(params["items"])*/) {
        (function (n, d) {
            ajax(d.getAttribute("sb-ajax"), "POST", params, function(responseText, response) {
                ...
                _sbecky_call_onresponse(n, responseText)
            }, function(e, response) {
...
function _sbecky_load(elem, succ=undefined, err=undefined) {
    ...
    if (parent.getAttribute("sb-ajax") == undefined) {
        ...
        (function(n, d, params){
            ajax(d.getAttribute("sb-ajax"), "POST", params, function(response, resp) {
...
function _sbecky_button_onclick(e) {
    ...
    (function(self){
        ...
        ajax(url, "POST", params, function(response, resp) {
            //Todo: handle callback
            // self.innerHTML = response
            // var r = document.createTextNode(response)
            // self.parentNode.replaceChild(self, r)

            var bindings = _sbecky_bindings[self.getAttribute("sb-button")]
            if (!bindings) {
                console.log("sbecky: Error! Binding '"+self.getAttribute("sb-button")+"' not found");
            }

            _sbecky_remove_placeholder(bindings[0])
            _sbecky_remove_error_placeholder(bindings[0])
...
window.addEventListener('load', _sbecky_start)
// window.onload =
...
function sbecky_change(variable, f = function(){}) {
    if(_sbecky_debug_register) console.log("sbecky: register onchange", variable);
    if (_sbecky_change_listeners[variable] == undefined){
...
function sbecky_onassign(variable, f = function(){}) {
    if(_sbecky_debug_register) console.log("sbecky: register onassign_override", variable);
    _sbecky_onassign_override[variable] = f
}

function sbecky_onsubmit(variable, f = function(){}) {
    if(_sbecky_debug_register) console.log("sbecky: register onsubmit", variable);
    if (_sbecky_onsubmit_listeners[variable] == undefined){
...
function sbecky_onclick(variable, f = function(){}) {
    if(_sbecky_debug_register) console.log("sbecky: register onclick", variable);
    if (_sbecky_onclick_listeners[variable] == undefined){
...
function sbecky_onresponse(variable, f = function(){}) {
    if(_sbecky_debug_register) console.log("sbecky: register onresponse", variable);
    if (_sbecky_onresponse_listeners[variable] == undefined){
...
function sbecky_onresponse_error(variable, f = function(){}) {
    if(_sbecky_debug_register) console.log("sbecky: register onresponse_error", variable);
    if (_sbecky_onresponse_error_listeners[variable] == undefined){
